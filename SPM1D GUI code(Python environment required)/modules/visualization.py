...
def plot_spm_result(spm_result, inference_result, ax=None, save_path=None, test_type='ttest', two_tailed=True, method='param'):
    setup_plot_style()
    ...
    prefix = 'SPM' if method == 'param' else 'SnPM'
    if test_type in ['ttest', 'ttest2', 'ttest_paired', 'regress']:
        ax.set_ylabel(f'{prefix}{{t}}', fontsize=12)
    elif test_type in ['anova1', 'anova1rm', 'anova2', 'anova3', 'anova2rm', 'anova2onerm']:
        ax.set_ylabel(f'{prefix}{{F}}', fontsize=12)
    else:
        ax.set_ylabel(f'{prefix}{{z}}', fontsize=12)
