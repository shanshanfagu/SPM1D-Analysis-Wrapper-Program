...
def export_all_to_xlsx(summary, normality_results, posthoc_summary,
    ...
    with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
        if summary:
            ...
            test_type = summary.get('test_type', 'N/A')
            
            if test_type in ['anova2', 'anova2rm', 'anova2onerm'] and 'effects' in summary:
                type_text = tr('export.two_way_anova_rm') if test_type in ['anova2rm', 'anova2onerm'] else tr('export.two_way_anova')

                rows = [
                    {tr('export.param'): tr('export.param_type'), tr('export.value'): type_text},
                ...
                for i, effect in enumerate(summary.get('effects', [])):
                    ...
                    rows.append({tr('export.param'): effect_name, tr('export.value'): ''})
                    rows.append({tr('export.param'): tr('export.param_threshold'), tr('export.value'): f"{effect.get('zstar', 0):.4f}" if effect.get('zstar') else 'N/A'})
                    rows.append({tr('export.param'): tr('export.param_h0reject'), tr('export.value'): h0_text})
                    rows.append({tr('export.param'): tr('export.param_clusters'), tr('export.value'): effect.get('n_clusters', 0)})
                ...
                if cached_spm_result and cached_inference_result and isinstance(cached_inference_result, list):
                    ...
                    for idx, (spm_f, inf_f) in enumerate(zip(cached_spm_result, cached_inference_result)):
                        ...
                        spm_df.to_excel(writer, sheet_name=effect_name, index=False, startrow=0, startcol=0)
            else:
